...
/**
 * Check if fb device is accessible and try mounting devtmpfs on /dev otherwise.
 * This is needed in case the kernel was compiled without DEVTMPFS_MOUNT symbol.
 *
 * @param dev fb device to check for. Must be a file under /dev.
 *
...
int ds_fs_setup(const char *dev)
{
    ...
    err("Failed to find a framebuffer -- %m");
    return -1;
}
